                ...
                <div className="students">
                        ...
                        <div className="year-month-wrapper">
                            ...
                            <div className="select-small-wrapper">
                                <select value={selectedMonth} onChange={handleMonthChange}>
                                    {days && days[selectedYear] && Object.keys(days[selectedYear]).map(month => (
                    ...
                    <table>
                        <thead>
                            <tr>
                                <th>Student</th>
                                {days && days[selectedYear] && days[selectedYear][selectedMonth] && days[selectedYear][selectedMonth].map((day, index) => (
                                    <th key={index}> {day} </th>
                                ))}
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            {students.length > 0 && students.map((student, index) => (
                                <tr key={index}>
                                    ...
                                    {days && days[selectedYear] && days[selectedYear][selectedMonth] && days[selectedYear][selectedMonth].map((day, index) => (
                                        <td key={index} className='day-cell' onClick={() => toggleAttendance(student.id, selectedYear, selectedMonth, day)}>
                                            {
                                                attendance[`${selectedYear}-${monthNumbers[selectedMonth]}-${String(day).padStart(2, "0")}_${student.id}`] === "present" ? <FaCheck /> :
                                                attendance[`${selectedYear}-${monthNumbers[selectedMonth]}-${String(day).padStart(2, "0")}_${student.id}`] === "absent" ? <FaX /> :
                                                attendance[`${selectedYear}-${monthNumbers[selectedMonth]}-${String(day).padStart(2, "0")}_${student.id}`] === "late" ? <FaClockRotateLeft /> :
                                                ""
                                            }
                                        </td>
                                    ))}
